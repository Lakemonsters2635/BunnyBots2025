...
public class PidAutoCommand extends Command {
  ...
  private final double m_xPrime, m_zPrime, m_finalYa;
    ...
    public PidAutoCommand(DrivetrainSubsystem drivetrainSubsystem, ObjectTrackerSubsystem objectTrackerSubsystem, int tagID, double xPrime, double zPrime, double finalYa, boolean isVisionAuto) {
      ...
      Pose2d targetPose = isVisionAuto ? visionAutoData(xPrime, zPrime, finalYa, tagID) : m_dts.getPose();
    ...
    public PidAutoCommand(DrivetrainSubsystem dts, ObjectTrackerSubsystem ots, double xTarget, double yTarget, double rotTarget) {
      this(dts, ots, -1, 0, 0, 0, false); //Sets visionOffsets to 0 to not intefere with targetPose calcs
      m_x_target = xTarget;
      m_y_target = yTarget;
  ...
  public void execute() {
    ...
    double speed_clamp = MathUtil.clamp(PURE_VISION_MAX_M_PER_SEC * (distanceFromStart / fadeInDistance), 0.5, PURE_VISION_MAX_M_PER_SEC);
    ...
    double pid_rot = m_visionSwerveController_rot.calculate(Math.toRadians(rot_pose), Math.toRadians(m_rot_target));
  ...
  private Pose2d visionAutoData(double xPrime, double zPrime, double finalYa, int tagId) {
    ...
    for (int i = 0; i < 100; i++) {
      try {
        // detection = YOLO_object == null ? (tagId == -1 ? m_ots.getNearestAprilTagDetection() : m_ots.getSpecificAprilTag(tagId)) : m_ots.getNearestYoloDetection();
        break;
      } catch (Exception e) {
